...
class DatabaseManager:
    def __init__(self):
        self.db_path = "data/database.db" # db olusturuldu
    ...
    async def does_server_exists(self, server_id): # sunucu yoksa ekle
        async with self.get_db() as db:
            await db.execute("INSERT OR IGNORE INTO server (server_id) VALUES (?)", (server_id,))
            await db.commit()

    async def does_user_exists(self, server_id, user_id): # kullanıcı yoksa ekle
        async with self.get_db() as db:
    ...
    # --- VERİ EKLEME ÇIKARMA (matematik iste) ---
    ...
    async def add_voice(self, server_id, user_id, seconds):
        await self.does_user_exists(server_id, user_id)
        add_xp = int(seconds / 60) * 2 # dakika basi 2 xp
        async with self.get_db() as db:
            await db.execute("""
    ...
    # level xp ayarlama (admin icin)

    async def set_user_level(self, server_id, user_id, level): # name mismatch fix: called set_user_level in stats.py
        await self.does_user_exists(server_id, user_id)
        async with self.get_db() as db:
            await db.execute("""UPDATE users SET level=? WHERE server_id=? AND user_id=?""", (level, server_id, user_id))
            await db.commit()

    async def set_user_xp(self, server_id, user_id, xp): # name mismatch fix
        await self.does_user_exists(server_id, user_id)
        async with self.get_db() as db:
